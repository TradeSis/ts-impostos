...
if (isset($jsonEntrada['xml'])) {
    ...
    foreach ($xmlArquivos as $xmlContent) {
        ...
        //********************************************PESSOAS
            ...
            $dadosEmpresa = mysqli_fetch_array($buscaEmpresa, MYSQLI_ASSOC);
            
            $sqlPessoaEmpresa = "SELECT * FROM pessoas WHERE idPessoa = " . $dadosEmpresa["idPessoa"];
            $buscaPessoaEmpresa = mysqli_query($conexao, $sqlPessoaEmpresa);
            $dadosPessoaEmpresa = mysqli_fetch_array($buscaPessoaEmpresa, MYSQLI_ASSOC);

            if ($emitCpfCnpj !== $dadosPessoaEmpresa["cpfCnpj"] && $destCpfCnpj !== $dadosPessoaEmpresa["cpfCnpj"]) {
                $jsonSaida = array(
                    "status" => 400,
